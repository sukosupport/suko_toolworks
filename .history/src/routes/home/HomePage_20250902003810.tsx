...
type ContactStored = ContactPayload & { createdAt: string }
...
function ContactSection() {
  ...
  async function sendFormData(payload: ContactPayload) {
    // ここで後日サーバーのエンドポイントへPOSTする処理に差し替え
    // 現状はローカル保存＆疑似送信
    try {
      const raw = localStorage.getItem('contact-drafts')
      const existing: ContactStored[] = raw ? JSON.parse(raw) : []
      const stored: ContactStored = { ...payload, createdAt: new Date().toISOString() }
